...
void drawEquipUIBefore(float dt, iPopup* pop)
{
	for (int i = 0; i < equipUI->slotMaxCnt; i++)
	{
		if (equipUI->slotIndexes[i] != -1)
		{
			...
			for (int j = 0; j < 2; j++)
				equipUI->strSlots[i][j]->setString("%d/%d/%d/%d", sl->itemIndex, sl->isEquipped, sl->itemCount, j);
		}
		else
		{
			for (int j = 0; j < 2; j++)
		...
		equipUI->imgSlots[i]->frame = (i == equipUI->selectedSlot || i == equipUI->selectingSlot);
	}
